...
from dolfin import FunctionSpace, grad, inner, sqrt, sym

__all__ = ["nn_setup", "nn_update"]


def nn_setup(
    u_, mesh, ModifiedCross, CG1Function, nu_nn_krylov_solver, bcs, **NS_namespace
):
    """
    ...
    nu_nn_form = (
        (mu_o - mu_inf) / ((1 + (lam * SII) ** m_param) ** a_param) + mu_inf
    ) / rho
    # bcs_nu_nn = derived_bcs(CG1, bcs['u0'], u_)
    nunn_ = CG1Function(
        nu_nn_form, mesh, method=nu_nn_krylov_solver, bounded=True, name="nu_nn"
    )
