...
from .DynamicModules import compute_Lij, compute_Mij, lagrange_average, tophatfilter

__all__ = ["les_setup", "les_update"]


def les_setup(
    u_, mesh, assemble_matrix, CG1Function, nut_krylov_solver, bcs, **NS_namespace
):
    """
    ...
    delta = pow(CellVolume(mesh), 1.0 / dim)
    ...
    nut_ = CG1Function(
        nut_form, mesh, method=nut_krylov_solver, bcs=bcs_nut, bounded=True, name="nut"
    )
    ...
    G_under.vector().set_local(1.0 / G_under.vector().array())
...
def les_update(
    ...
    **NS_namespace
    ...
    tophatfilter(unfiltered=Cs, filtered=Cs, N=2, weight=1.0, **vars())
